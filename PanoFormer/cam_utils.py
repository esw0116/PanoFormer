...
def sphere2uv(coord_cam, color_cam, H, W, K):
    # coord_cam: (3, N), N: # of points, (X, Y, Z)
    # color_cam: (3, N), (R, G, B)

    N = coord_cam.shape[1]
    dist = np.sqrt(np.sum(np.power(coord_cam, 2), axis=0)+1e-6)
    dist_xz = np.sqrt(np.sum(np.power(coord_cam[[0,2]], 2), axis=0)+1e-6)

    lat = np.arctan2(-coord_cam[1], dist_xz)
    lon = np.arctan2(coord_cam[0], coord_cam[2])
    ...
    omni_img = griddata(coord_uv[:2].transpose(1,0), color_cam, grid, method='linear', fill_value=0).reshape(H,W,3)
